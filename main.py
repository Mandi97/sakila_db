...
query_delete = sql.SQL("""DELETE FROM {table_name} WHERE id=%s""").format(table_name=sql.Identifier('User'))

query_update = sql.SQL("""UPDATE {table_name} SET email=%s WHERE id=%s""").format(table_name=sql.Identifier('User'))
...
query_alter2 = sql.SQL(
    """ALTER TABLE {table_name} ADD COLUMN date_of_created TIMESTAMP DEFAULT CURRENT_TIMESTAMP""").format(
    table_name=sql.Identifier('Address'))

query_select = sql.SQL("""SELECT * FROM {table_name}""").format(table_name=sql.Identifier('User'))

try:
    with cnx, cnx.cursor() as cs:
        # try:
        #     # cs.execute(query_create_tb)
        #     cs.execute(query_create_tb_address)
        # except DatabaseError as err:
        #     print(err)

        try:
            cs.execute(query_insert, ('ala', 'b@a.pl', 'secret'))
        except DatabaseError as err:
            print(err)
        ...
        # try:
        #     cs.execute(query_update, ('m.morawiecki@onet.pl', 2))
        # except DatabaseError as err:
        #     print(err)
